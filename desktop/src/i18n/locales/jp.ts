...
export const jp: Record<TranslationKey, string> = {
  // ── auto-filled from en (2 missing keys — translate me ──
  'settings.general.webSearch.mode.madcop': 'Madcop',
  'settings.mcp.scope.madcopai': 'MadcopAI',

  // ── auto-filled from en (164 missing keys — translate me ──
  ...
  'sidebar.createBlankSession': '空のセッションを作成',
  ...
  'settings.activity.subtitleLoading': 'ローカルの MadCop Agent CLI セッション記録に基づきます',
  ...
  'settings.terminal.description': 'プラグイン、スキル、MCP のセットアップ用にホストマシンのコマンドを実行します。デスクトップアプリには madcop-haha が含まれています。ドキュメントの madcop <args> を madaude-haha <args> に置き換えてください。例: claude-haha plugin install ... や claude-haha mcp add ...',
  ...
  'settings.diagnostics.doctorSafeKeys': 'クリアされるのは次のみ: madcop-agent-open-tabs、madcop-agent-session-runtime、madcop-agent-theme、madcop-agent-locale、madcop-agent.persistence.schemaVersion。',
  ...
  'settings.providers.customIntro': 'MadCop には MiniMax がデフォルトで設定されています。下のボタンをクリックして、カスタムモデルプロバイダーを管理します。',
    'settings.providers.customLogin.intro': 'MadCop には MiniMax がデフォルトで設定されています。下のボタンをクリックして、カスタムモデルプロバイダーを管理します。',
  'settings.providers.customLogin.loginButton': 'カスタムモデルを追加',
  'settings.providers.customLogin.loginStarting': '起動中…',
  'settings.providers.customLogin.logoutButton': 'サインアウト',
  'settings.providers.customLogin.logoutProcessing': '処理中…',
  'settings.providers.customLogin.loggedInPrefix': 'サインイン済み (MadCop',
  'settings.providers.customLogin.subTypeUnknown': '不明',
  'settings.providers.customLogin.errorPrefix': 'エラー: ',
  'settings.providers.customLogin.openBrowserFailed': 'ブラウザを開けませんでした。承認 URL に手動でアクセスしてください。',
  ...
  'settings.providers.settingsJsonDesc': '~/.madcop/madcop-agent/settings.json — 直接編集できます。保存時に書き込まれます。',
  ...
  'settings.adapters.description': 'Telegram、WeChat、DingTalk、WhatsApp、Feishu 経由で MadCop Agent とチャットできるよう IM アダプターを設定します。詳しい設定手順は',
  ...
  'settings.mcp.description': 'MadCop Agent 用に外部ツールやデータソースを接続します。Local、Project、User の各スコープは CLI と同じ動作に従います。',
  ...
  'settings.mcp.form.createHint': '現在 MadCop Agent がサポートするフィールドでカスタム MCP サーバーをセットアップします。',
  ...
  'settings.mcp.targetProject.globalHint': 'User スコープは対象プロジェクトを必要としません。グローバルの MadCop 設定に保存され、どこからでも利用できます。',
  ...
  'settings.agents.description': '組み込み、プロジェクト、ユーザーの各ソースにわたって MadCop が利用できるエージェントを閲覧します。TUI で /agents を使うと、アクティブなソースとオーバーライドを確認できます。',
  ...
  'settings.skills.description': 'スキルは MadCop に専門的な機能を追加します。スキルは ~/.madcop/skills で管理します',
  ...
  'settings.skills.emptyHint': '~/.madcop/skills にスキルを追加して始めましょう',
  ...
  'settings.memory.description': 'Claude が各プロジェクト向けに書き込む Markdown メモリファイルを確認・編集します。これらのファイルは ~/.madcop/projects/<project>/memory/ の下に保存され、CLI ランタイムによって読み込まれます。',
  ...
  'settings.plugins.emptyHint': 'MadCop Agent からプラグインをインストールすると、ここで管理できます。',
  ...
  'settings.computerUse.appsDescription': 'コンピューター操作用にアプリを事前承認します。承認済みアプリは、実行時の確認なしに MadCop が操作できます。',
  ...
  'settings.general.storagePortableDirPlaceholder': 'madcop-agent データ用のフォルダを選択',
  ...
  'settings.general.storageMoveHint': 'ディレクトリを切り替えても既存のデータは移行されません。以前のセッションを引き続き表示したい場合は、古いディレクトリから projects、skills、plugins、madcop-agent および関連フォルダをコピーしてください。ポータブルバンドルにする場合は、このフォルダをアプリの隣に置いて一緒に zip 化してください。',
  ...
  'settings.general.outputStyleDescription': '新しいセッションまたは再起動後のセッションで MadCop がどのように応答するかを選択します。',
  ...
  'settings.general.outputStyleScopeLocalHint': 'アクティブなプロジェクトの .madcop/settings.local.json に保存されます。',
  ...
  'settings.general.notificationsTestTitle': 'MadCop Agent の通知が有効になりました',
  ...
  'settings.general.traceHintOn': '新しいセッションは、簡潔な要求、応答、状態イベントをローカルの madcop-agent traces ディレクトリに書き込みます。',
  ...
  'settings.general.webFetchPreflightDescription': 'デスクトップセッションは、サードパーティプロバイダーや制限されたネットワークでの誤った失敗を避けるため、デフォルトで MadCop のドメインプリフライトをスキップします。',
  ...
  'settings.general.webSearchDescription': 'エージェントのウェブ検索を、公式 MadCop、サードパーティプロバイダー、ローカルのフォールバックキーのどれで解決するかを選択します。',
  ...
  'settings.general.webSearchHint': '自動の場合、MadCop のモデル名にはネイティブの MadCop ウェブ検索を使用し、その後 Tavily と Brave のキーにフォールバックします。',
  ...
  'empty.subtitle': '新しいコーディングセッションを開始します。MadCop は、プロジェクトの構築、デバッグ、設計をお手伝いする準備ができています。',
  ...
  // ─── Chat Input ──────────────────────────────────────
  'chat.placeholder': '編集、デバッグ、説明を MadCop に依頼...',
  ...
  'computerUseApproval.hideWhileWorking': 'MadCop の作業中、他の {count} 個のアプリが非表示になります。',
  'computerUseApproval.hideWhileWorkingRestore': 'MadCop の作業中、他の {count} 個のアプリが非表示になり、Claude の完了後に復元されます。',
  ...
  'computerUseApproval.tryAgainHint': '再試行すると MadCop に制御が戻り、macOS の権限変更が反映された後に request_access をもう一度呼び出せます。',
