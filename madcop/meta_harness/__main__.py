"""CLI: python -m madcop.meta_harness [run|status|promote|axes]

Examples::

    python -m madcop.meta_harness status
    python -m madcop.meta_harness run --iterations 5
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "run":
        ...
        loop = MetaHarnessLoop(seed=args.seed, promote_best=args.promote)
    ...
    if args.cmd == "promote":
        ...
        if args.id:
            ...
            if not score_path.exists():
                # try prefix match
                matches = [p for p in arch.root.iterdir() if p.is_dir() and p.name.startswith(args.id)]
                if not matches:
