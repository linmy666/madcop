...
def _build_memory_system_prompt(
    ...
    *,
    ...
    parts: list[str] = [
        ...
        "PRESENTATION & DOCUMENT CRAFT (apply silently when building "
        "decks or documents with write_pptx/write_xlsx/write_file): "
        "structure decks as cover → context/evidence → data page → "
        "risks/actions → closing (5-7 pages); pick a different layout "
        "kind per page so adjacent pages differ; lead data pages with "
        "table/chart/kpi layouts instead of bullet walls; verify every "
        "number you put on a slide against its source before writing; "
        "end answers that delivered files with real paths only.\n\n"
...
def create_app() -> FastAPI:
    ...
    # Client cache: avoid rebuilding OpenAICompatClient (and its
    # underlying httpx.Client connection pool) on every /api/chat
    # request. Keyed on (provider_id, api_key, base_url, model) so a
    # settings change invalidates the cache automatically.
    ...
    # ── DEPRECATED /api/chat handler REMOVED ──────────────────────
    # The 1,850-line legacy SSE handler was deleted. The default UI
 # posts to /api/v4/chat (routes/chat_v4.py). DirectChatButton and
    # the static web demo were migrated. Tests that pinned this route
    # were migrated to /api/v4/chat or removed.
    # ────────────────────────────────────────────────────────────────
    ...
    # ── WebSocket /ws/{session_id} handler REMOVED ─────────────────
    # No frontend consumer existed (the React UI was deleted; the
    # Vue app uses /api/v4/chat SSE). Live transport is SSE only.
    # ────────────────────────────────────────────────────────────────
